"""Tests para src/rubros/instalacion_sanitaria.py"""
...
def _calc(**kwargs):
    return REGISTRY["instalacion_sanitaria"].calcular(
        ParamsInstalacionSanitaria(**kwargs), EMPRESA
    )


class TestCasosBase:
    def test_caso_base_bano(self):
        r = _calc(cantidad_banos=1)
        assert r.metadata["cantidad_banos"] == 1
        assert r.total > 0

    def test_dos_banos(self):
    ...
    def test_bano_cocina(self):
        ...
        assert r.metadata["cantidad_banos"] == 1
        assert r.metadata["cantidad_cocinas"] == 1

    def test_invariante_suma_igual_total(self):
        r = _calc(cantidad_banos=2, cantidad_cocinas=1)
...
class TestValidacion:
    def test_cantidad_banos_min_falla(self):
