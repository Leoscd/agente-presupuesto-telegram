...
class TestCasosBase:
    def test_asfaltica(self):
        r = _calc(superficie_m2=30.0, tipo="asfaltica", capas=2)
        assert r.metadata["tipo"] == "asfaltica"
        assert r.total > 0
...
class TestPropiedades:
    ...
    def test_idempotencia(self, sup):
        r1 = _calc(superficie_m2=sup, tipo="asfaltica", capas=2)
        r2 = _calc(superficie_m2=sup, tipo="asfaltica", capas=2)
